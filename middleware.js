...
const SOURCES = {
  ...
  "/lokali-prestataires.html":  { param: "presta",     table: "prestataires",                     titreCol: "nom",         descCol: "description", photosCol: null, photoUniqueCol: "photo_url" },
  "/lokali-emplois.html":       { param: "offre",      table: "lokt_emplois",                     titreCol: "titre_poste", descCol: "description", photosCol: "photos" }
};
...
export default async function middleware(request) {
  ...
  // Cas particulier : lien de parrainage sur la page d'accueil (?ref=CODE),
  // logique différente du reste (pas un ID d'annonce, mais un code de parrainage,
  // et l'info vient d'une fonction Supabase sécurisée, pas d'une table directe).
  if (pathname === "/") {
    const code = url.searchParams.get("ref");
    if (!code) return;
    try {
      const rpcUrl = SUPA_URL + "/rest/v1/rpc/lokali_info_parrain";
      ...
      const info = Array.isArray(data) && data[0] ? data[0] : null;
      if (!info) return;

      const nom = info.nom_affichage || "Un membre LOKALI";
      const boutiqueTxt = info.boutique_nom ? " (" + info.boutique_nom + ")" : "";
      const photo = info.boutique_logo || "https://mylokali.com/lokali-logo-v2.svg";
      const echapper = (t) => t.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

      const html =
        "<!DOCTYPE html><html lang='fr'><head><meta charset='UTF-8'>" +
        "<title>" + echapper(nom) + " t'invite sur LOKALI</title>" +
        "<meta property='og:title' content='" + echapper(nom) + boutiqueTxt + " t\\'invite à rejoindre LOKALI'>" +
        "<meta property='og:description' content='Publie tes annonces, ouvre ta boutique, trouve un professionnel — gratuitement, partout en Afrique francophone.'>" +
