...
(function(){
  ...
  function initGate() {
    ...
    // Lien reçu avec ?code=... → on pré-remplit et on vérifie automatiquement,
    // le sponsor/partenaire n'a rien à retaper.
    var codeUrl = new URLSearchParams(window.location.search).get("code");
    if (codeUrl) {
      document.getElementById("lokaliGateCode").value = codeUrl;
      verifierCode();
    }
  }
