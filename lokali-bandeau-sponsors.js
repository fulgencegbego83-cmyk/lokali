...
(function(){
  ...
  style.textContent =
    /* ═ Vitrine compacte façon "widgets" (Officiel + Diamant, en rangée) ═
       Chaque carte est proportionnée à son rang, sans espace mort : logo en
       haut (grand pour Officiel, plus petit pour Diamant), nom et badge juste
       en dessous, tout est compact et bien rempli. */
    ".lka-vitrine{max-width:680px;margin:10px auto;padding:0 14px;display:flex;gap:10px;overflow-x:auto}" +
    ".lka-carte-sp-inner{flex-shrink:0;display:flex;flex-direction:column;align-items:flex-start;background:#fff;" +
    "border:1px solid rgba(15,23,42,.09);border-radius:12px;box-shadow:0 1px 4px rgba(15,23,42,.05);" +
    "text-decoration:none;padding:12px 14px;box-sizing:border-box}" +
    ".lka-carte-sp-inner:active{background:rgba(15,23,42,.02)}" +
    ".lka-carte-sp-inner.grand{width:184px}" +
    ".lka-carte-sp-inner.moyen{width:152px}" +
    ".lka-carte-sp-logo{width:100%;display:flex;align-items:center;justify-content:flex-start;margin-bottom:8px}" +
    ".lka-carte-sp-inner.grand .lka-carte-sp-logo{height:66px}" +
    ".lka-carte-sp-inner.moyen .lka-carte-sp-logo{height:52px}" +
    ".lka-carte-sp-logo img{max-width:100%;max-height:100%;object-fit:contain}" +
    ".lka-carte-sp-eyebrow{font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#94A3B8;margin-bottom:3px}" +
    ".lka-carte-sp-badge{display:inline-flex;align-items:center;gap:3px;font-size:10px;font-weight:800;border-radius:20px;padding:2px 9px;margin-bottom:5px;white-space:nowrap}" +
    ".lka-carte-sp-nom{font-weight:800;color:#1E293B;width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}" +
    ".lka-carte-sp-inner.grand .lka-carte-sp-nom{font-size:15px}" +
    ".lka-carte-sp-inner.moyen .lka-carte-sp-nom{font-size:13px}" +
    ".lka-carte-sp-cta{font-size:10.5px;font-weight:700;margin-top:3px}" +
  ...
  function creerCarteSponsor(s, palette, taille) {
    ...
    var labelBadge = (estPartenaire(s) ? "🤝 " : "🏆 ") + s.formule;

    var el = document.createElement("a");
    el.className = "lka-carte-sp-inner " + taille;
    el.href = lien;
    el.target = "_blank";
    el.rel = "noopener";
    el.innerHTML =
  ...
  function construireBlocs(tous) {
    ...
    var vitrineCartes = [];
    sponsorOfficiel.forEach(function(s){ vitrineCartes.push(creerCarteSponsor(s, ORANGE, "grand")); });
    partenaireOfficiel.forEach(function(s){ vitrineCartes.push(creerCarteSponsor(s, BLEU, "grand")); });
    sponsorDiamant.forEach(function(s){ vitrineCartes.push(creerCarteSponsor(s, ORANGE, "moyen")); });
    if (vitrineCartes.length) {
      var vitrine = document.createElement("div");
      vitrine.className = "lka-vitrine";
      vitrineCartes.forEach(function(c){ vitrine.appendChild(c); });
      blocs.push(vitrine);
    }
