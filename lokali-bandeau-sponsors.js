...
(function(){
  ...
  style.textContent =
    ...
    /* ═ Niveau 1 : GRANDE bannière (façon pub pleine largeur) — Officiel uniquement ═ */
    ".lka-banniere{max-width:680px;margin:10px auto;padding:0 12px}" +
    ".lka-banniere-inner{position:relative;display:block;height:170px;border-radius:18px;overflow:hidden;text-decoration:none;box-shadow:0 10px 30px rgba(0,0,0,.25)}" +
    ".lka-banniere-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}" +
    ".lka-banniere-fallback{position:absolute;inset:0}" +
    ".lka-banniere-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.75) 0%,rgba(0,0,0,.25) 55%,rgba(0,0,0,.05) 100%)}" +
    ".lka-banniere-glow{animation:lkaBanGlow 2.6s ease-in-out infinite}" +
    "@keyframes lkaBanGlow{0%,100%{box-shadow:0 10px 30px rgba(0,0,0,.25)}50%{box-shadow:0 10px 40px rgba(0,0,0,.25),0 0 0 4px var(--lka-ombre)}}" +
    ".lka-banniere-badge{position:absolute;top:14px;left:16px;font-size:12px;font-weight:900;letter-spacing:.4px;color:#fff;background:var(--lka-accent);border-radius:20px;padding:5px 14px;box-shadow:0 4px 12px rgba(0,0,0,.3)}" +
    ".lka-banniere-nom{position:absolute;bottom:14px;left:16px;right:16px;color:#fff;font-size:22px;font-weight:900;text-shadow:0 2px 10px rgba(0,0,0,.6)}" +
    ".lka-banniere-cta{position:absolute;bottom:16px;right:16px;font-size:11.5px;font-weight:800;color:#fff;background:rgba(255,255,255,.18);backdrop-filter:blur(6px);border:1px solid rgba(255,255,255,.35);border-radius:20px;padding:6px 14px}" +
    /* ═ Niveau 2 : bannière sobre compacte (Diamant sponsor) ═ */
  ...
  function creerBanniereGeante(s, palette) {
    var lien = lienValide(s.site_web) || lienValide(s.lien_video) || "#";
    var logo = (s.photos && s.photos.length) ? s.photos[0] : null;
    var labelBadge = (estPartenaire(s) ? "🤝 PARTENAIRE OFFICIEL" : "🏆 SPONSOR OFFICIEL");

    var wrap = document.createElement("div");
    wrap.className = "lka-banniere";
    var html = "<a class='lka-banniere-inner lka-banniere-glow' href='" + lien + "' target='_blank' rel='noopener' style='--lka-ombre:" + palette.ombre + ";--lka-accent:" + palette.texte + "'>";
    if (logo) {
      html += "<img class='lka-banniere-bg' src='" + logo + "' alt='" + s.entreprise + "'>";
    } else {
      html += "<div class='lka-banniere-fallback' style='background:" + palette.fond + "'></div>";
    }
    html += "<div class='lka-banniere-overlay'></div>" +
      "<span class='lka-banniere-badge' style='background:" + palette.texte + "'>" + labelBadge + "</span>" +
      "<div class='lka-banniere-nom'>" + s.entreprise + "</div>" +
      "<span class='lka-banniere-cta'>Découvrir →</span>" +
    "</a>";
    wrap.innerHTML = html;
    return wrap;
  }
  ...
  // Construit la liste ordonnée des blocs (bannières, rangées, bandeau défilant)
  // pour un lot de sponsors/partenaires donné — réutilisable à plusieurs endroits
  // de la page (haut ET milieu, par exemple).
  function construireBlocs(tous) {
    ...
    sponsorOfficiel.forEach(function(s){ blocs.push(creerBanniereGeante(s, ORANGE)); });
    partenaireOfficiel.forEach(function(s){ blocs.push(creerBanniereGeante(s, BLEU)); });
    sponsorDiamant.forEach(function(s){ blocs.push(creerVedette(s, ORANGE, false)); });
  ...
  async function initBandeau() {
    ...
    // ═ Emplacement 1 : juste sous le menu (comportement historique) ═
    ...
    // ═ Emplacement 2 (optionnel) : n'importe où au milieu d'une page ═
    // Il suffit de placer <div id="lokaliSponsorsMilieu"></div> à l'endroit
    // voulu dans le HTML de la page pour que la même hiérarchie s'y affiche
    // aussi — indépendamment du bloc du haut, avec ses propres liens cliquables.
